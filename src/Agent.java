...
public class Agent {
	...
	public Agent (World world, String type, double alpha, double gamma, State init_state)
	{
		this.world = world;
	...
	public int pick_next_action (State state, int maxruns, int currentrun)
	{
		int result = -1;
	...
	public void single_step (State state, int action) 
	{
	...
	public void single_step (State state) 
	{
		State current_state = state;
		int next_action;
	...
	public void single_run (State state, int maxruns, int currentrun) 
	{
		...
		while (!current_state.equals(this.getGoalState())) //this.goalState)
		{
			...
			/*if (next_action == 3) 
			{	
				State temp = new State(1,0);
				current_state.print();
				if (current_state.equals(temp))
					end_state = this.world.transitionTo(current_state, next_action);
				else 
					end_state = this.world.transitionTo(new State(current_state.getX(), current_state.getY()), next_action);
			}			
			else*/			
			end_state = this.world.transitionTo(current_state, next_action);
			if (debug)	
	...
	public void multiple_runs (int count, State state)
	{
	...
	public void Q_update (State currentState, int action)
	{
		if (debug)
	...
	public void Q_update (State currentState, int action, State endState)
	{
		if (debug)
			System.out.println("Updating");
	...
	public void A_update (State currentState, int action)
	{
	...
	public void setCurrentState(State q)
	{
		this.currentState = q;
	}
	...
	{
		World world = new World(new State (2,2));

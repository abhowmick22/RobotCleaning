...
public class World {
	...
	// state is pair <x,y>
	// default 3x3 table
	private State[] listOfStates = new State [9];
	// size of X-dimension
	private int xSize = 3;
	// size of Y-dimension
	private int ySize = 3;
	...
	// (0,1)
	private State[][] Walls = new State [1][2];
	...
	// deterministic world
		...
		{ 
			...
			for (int i=0; i<3;i++)
				for (int j=0; j<3; j++)
					listOfStates[(i*3)+j] = new State(i,j); // create table of pairs	
			...
			{	
				...
				if (state.getX()<2)
				{
					State temp = new State(state.getX()+1, state.getY());
				...
				if (state.getY()>0)
				{
					State temp = new State(state.getX(), state.getY()-1);
			...
			// init rewards
			this.rewards[this.stateToIndex(new State(2,2))] = 100.0;
			
			// add a wall
			this.Walls[0][0] = new State(0,0);
			this.Walls[0][1] = new State(0,1);
			
			//this.goalState = new State (2,2);
		}
		
	// typical constructor
	public World (State goalState)
	{
		this();
	...
	public int stateToIndex (State state)
	{
		return ((state.getX()*this.xSize)+state.getY());
	}
	
	// Go from int state to (x,y) coordinate
	public State indexToState (int state)
	{
		int factor = state/this.xSize;
		return (new State (factor, state-(this.xSize*factor)));
	}
	
	
	// return transition
	public State transitionTo (State q, int action )
	{
		State state_prime = null;
		for (State state_to : listOfStates)
		{
	...
	// return reward
	public double getReward (State q)
	{
	...
	public void printRewards()
	{
		...
		for (int i =this.xSize-1; i>= 0; i--)
		{
			...
			{
				System.out.print(this.getReward(new State(i,j))+" ");
			}
			System.out.println();
		}
